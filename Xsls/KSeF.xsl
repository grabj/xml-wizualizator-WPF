...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tns="http://crd.gov.pl/wzor/2025/06/25/13775/" version="1.0" exclude-result-prefixes="tns">
  ...
  <xsl:decimal-format name="european" decimal-separator=',' grouping-separator=' ' NaN=""/>
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="tns:Faktura">
				<xsl:apply-templates select="tns:Faktura"/>
			</xsl:when>
			<xsl:otherwise>
				<html lang="pl">
					<head>
						<meta charset="utf-8"/>
						<title>Błąd szablonu</title>
					</head>
					<body style="font-family: Arial, sans-serif; padding: 2em;">
						<h2>Wybrano niewłaściwy szablon</h2>
						<p>
							Ten szablon XSL obsługuje wyłącznie dokumenty e-Faktury KSeF.<br/>
							Wybierz odpowiedni szablon i spróbuj ponownie.
						</p>
					</body>
				</html>
			</xsl:otherwise>
