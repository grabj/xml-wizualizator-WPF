	...
	<xsl:template match="mstns:TabletPrzesylka">
		<table>
			...
			<tr>
				<th>Nadawca</th>
				<td>
					<xsl:value-of select="mstns:TabletJednostkaMS/mstns:NazwaJednostki"/>
					<br/>
					<xsl:value-of select="mstns:TabletJednostkaMS/mstns:Ulica"/>
					<xsl:text> </xsl:text>
					<xsl:value-of select="mstns:TabletJednostkaMS/mstns:Dom"/>
					<xsl:if test="string-length(normalize-space(mstns:TabletJednostkaMS/mstns:Lokal)) &gt; 0">
						<xsl:text> / </xsl:text>
						<xsl:value-of select="mstns:TabletJednostkaMS/mstns:Lokal"/>
					</xsl:if>
					<br/>
					<xsl:value-of select="mstns:TabletJednostkaMS/mstns:KodPocztowy"/>
					<xsl:text> </xsl:text>
					<xsl:value-of select="mstns:TabletJednostkaMS/mstns:Miasto"/>
				</td>
			</tr>
		...
		<div class="signature">
			...
			<xsl:if test="string-length(normalize-space(mstns:DaneBiometryczne/mstns:DataPodpisu)) &gt; 0">
				<div class="sig-date">
					Data podpisu:
					<xsl:call-template name="format-date">
